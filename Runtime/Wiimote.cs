...
namespace WiimoteApi
{
    ...
    public class Wiimote
    {
        ...
        /// If this Wiimote is a Balance board,
        /// this contains all relevant balance boardData as it is reported by
        /// the Controller.  If this Wiimote is not a Balance board, this is \c null.
        ///
        /// \sa current_ext
        public BalanceBoardData BalanceBoard
        {
            get
            {
                if (current_ext == ExtensionController.BALANCEBOARD)
                    return (BalanceBoardData)_Extension;
                return null;
            }
        }
        ...
        private const long ID_BalanceBoard              = 0x0000A4200402;
        ...
        private void RespondIdentifyExtension(byte[] data)
        {
            ...
            else if (val == ID_BalanceBoard)
            {
                _current_ext = ExtensionController.BALANCEBOARD;
                if (_Extension == null || _Extension.GetType() != typeof(BalanceBoardData))
                    _Extension = new BalanceBoardData(this);
            }
            else
            {
        ...
        //edit
        /// <summary>
        /// Set the wiimote LEDs to match the player number, using a filling led pattern upto 10 players
        /// </summary>
        /// <param name="nr">The current nr of the remote</param>
